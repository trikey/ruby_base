class RailwayDepot
  ...
  def start_work
    ...
    loop do
      ...
      case choice
        ...
        when 18 #Просмотреть список вагонов в поезде
          show_carriages_in_train
          
        when 19 #Занять место или объем в вагоне
          load_carriage
      end
    end
  end
  ...
  private
    ...
    def show_carriages_in_train
      ...
      raise "Поезда с таким номером нет" if train.nil?
      carriage_count = 0;
      train.iterate_carriages { |carriage| puts "№#{carriage_count += 1} #{carriage.type} свободно #{carriage.free}, занято #{carriage.taken}" }
      rescue RuntimeError => e
        puts "Ошибка: #{e.message}"
    end
